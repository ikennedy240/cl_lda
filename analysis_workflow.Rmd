...
fct_count(out$meta$neighborhood_type)
fct_count(fct_drop(cl_dropped$neighborhood_type))
```
...
fct_count(fct_relevel(fct_drop(out$meta$neighborhood_type), "predominantly white"))
```
...
map_cl <- model_fit %>% arrange(desc(Topic33)) %>% head(1000)
...
for(i in paste0('Topic',1:40)){
  test_model <- glm(get(i) ~ fct_relevel(fct_drop(neighborhood_type))+
                     pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
  ...
  #                   coefs = c(test_sum$coefficients[2,1],
  #                             test_sum$coefficients[3,1],
  #                             test_sum$coefficients[4,1]), 
  #                   stderrs = c(test_sum$coefficients[2,2],
  #                               test_sum$coefficients[3,2],
  #                               test_sum$coefficients[4,2])
  #                   )
  row <- data.frame(topic = i,
                    n_type = c('mixed','white asian','white black','white latinx','white mixed','majority non-white'),
...
test_coef_sum %>% 
  #left_join(read_csv('resources/topic_descriptions.txt')) %>%
  ...
  ggplot(aes(x = reorder(topic, -coefs), y = coefs))+
    ...
    geom_point(aes(color = n_type))+
    #geom_pointrange(aes(ymax = high_est, ymin = low_est, color=race), size=.2)+
    #geom_text(aes(y = coefs + (.0009*nchar(title)), label = description)) +
    #geom_text(aes(y = -(coefs/abs(coefs))*.08 + -(coefs/abs(coefs)*str_length(description)*.003), label = description), check_overlap = TRUE, family = 'Monaco', size = 2.5) +
    #geom_text(aes(y=0, label = topic), family = 'Monaco', size = 4)+
    #geom_label(aes(y=0, x = 15, label = "Topic Number"))+
    theme_minimal() + # auto exlude backgound shading
    theme(text = element_text(size=10))+ #, # set values for text
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
    ggtitle("Estimated Effects of neighborhood racial proportions on Topic Distributions (Race/Ethnicity Variables only)",
            subtitle = 'Showing all')+
    theme(text = element_text(size=10), #, # set values for text
          axis.text.y = element_blank(),
          axis.title.x = element_blank(),
          axis.title.y = element_blank()) +
    ylim(-max(abs(test_coef_sum$coefs))-.05,max(abs(test_coef_sum$coefs))+.05)+
...
for(i in paste0('Topic',1:40)){
  ...
  print(test_sum)
  row <- data.frame(topic = i,
                    race = c('black','asian','latinx','poverty'),
                    coefs = c(test_sum$coefficients[2,1],test_sum$coefficients[3,1],test_sum$coefficients[4,1], test_sum$coefficients[6,1]),
                    stderrs = c(test_sum$coefficients[2,2],test_sum$coefficients[3,2],test_sum$coefficients[4,2],test_sum$coefficients[6,2])
                    )
  test_coef_sum <- bind_rows(test_coef_sum, row)
}
test_coef_sum %>% left_join(read_csv('resources/topic_descriptions.txt')) %>%
  mutate(high_est = coefs + stderrs, low_est = coefs - stderrs, topic = factor(str_extract(topic, '\\d+'))) %>% 
  arrange(desc(coefs))  %>%
  filter(race!='poverty', topic %in% c(37, 2, 33,15,27,35)) %>%
  #filter(abs(coefs) > .05) %>% 
  ggplot(aes(x = reorder(topic, -coefs), y = coefs))+
    geom_hline(yintercept = 0, color = "red", alpha=.5) + # plot a line at 0
    geom_pointrange(aes(ymax = high_est, ymin = low_est, color=race, shape=race), size=.5)+
    #geom_text(aes(y = coefs + (.0009*nchar(title)), label = description)) +
    geom_text(aes(y = -(coefs/abs(coefs))*.08 + -(coefs/abs(coefs)*str_length(description)*.003), label = description), check_overlap = TRUE, family = 'Monaco', size = 2.5) +
    geom_text(aes(y=0, label = topic), family = 'Monaco', size = 4)+
    #geom_label(aes(y=0, x = 2, label = "Topic Number"))+
    theme_minimal() + # auto exlude backgound shading
    theme(text = element_text(size=10))+ #, # set values for text
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
    #ggtitle("Estimated Effects of neighborhood racial proportions on Topic Distributions (With Controls*)",
            #subtitle = 'Showing only selected topics')+
    theme(text = element_text(size=10), #, # set values for text
          legend.position = c(.1,.8),
          legend.title = element_blank(),
          legend.background = element_rect(fill='grey90', linetype = 0),
          axis.text.y = element_blank(),
          axis.title.x = element_blank(),
          axis.title.y = element_blank()) +
    ylim(-max(abs(test_coef_sum$coefs))-.05,max(abs(test_coef_sum$coefs))+.05)+
...
ggplot()+
  geom_histogram(aes(coef_sum$coefs, y = ..density..,fill = 'observed'), alpha = .5)+
...
merged_fit <- merged_fit %>% mutate(
  safety_terms = str_detect(rawcleanText, "safe|secure|secured|security|control|controlled|patrol|gate|gated|protect|protection|protected|intercom|police|alarm"),
...
test_model <- glm(safety_terms ~ black_proportion +
                       latinx_proportion+
                       asian_proportion+
                       pop_thousands + 
                       pov_proportion +
                       log_income +
                       log_price +
                       log_sqft, data = merged_fit, family = binomial())
summary(test_model)
```
...
test_model <- glm(SectySystem ~ black_proportion +
                       latinx_proportion+
                       asian_proportion+
                       pop_thousands + 
                       pov_proportion +
                       log_income, data = kc_tracts, family = binomial())
summary(test_model)
...
ggplot() +
  ...
  geom_point(aes(x = lng, y = lat), 
             ...
             size = .4,
             alpha = .2)+
  geom_sf(data=seabound, # Seattle Boundary
          color="black",
          size=.7,
          alpha = 0) +
...
ggsave('output/Plots/cl_map.png', device = png())
```
...
file_names <- c("before_7_1.txt", "7_1_to_2_19.txt", "after2_19.txt")
