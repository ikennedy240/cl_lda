...
top_texts <- map_dfr(paste0('Topic', 1:n_topics), prep_df, n=3, covariates = 'none',random=TRUE) # map through the topics and make a df
...
for(topic in unique(top_texts$topic)){
    ...
    filepath = paste0('output/Topics/',topic,'.txt') # include the topic in the filename
    # prepare file contents
...
test_coef_sum %>%  
  ...
  ggplot(aes(x = reorder(topic, -coefs), y = coefs))+
    geom_hline(yintercept = 0, color = "red", alpha=.5) + # plot a line at 0
    #geom_point(aes(color = n_type))+
    geom_pointrange(aes(ymax = high_est, ymin = low_est, color=n_type), size=.2, alpha = .8)+
    ...
    ylim(-max(abs(test_coef_sum$coefs))-.01,max(abs(test_coef_sum$coefs))+.01)+
...
```{r plot kc maps}
kc_cl_dots <- merged_fit %>% filter(GEOID10 %in% kc_nt$Tract_FIPS, Topic28>.15)
...
```{r}
merged_fit %>% filter(str_detect(listingText, 'police')) %>% select(listingText)
```
