...
cl_data <- cl_data %>%
  mutate(textAndTitle = paste(listingTitle, listingText, sep='\n\n'),
         cleanText = str_to_lower(cleanText), # convert to lower
...
for(i in 1:length(file_names)){
  frame <- cl_dropped %>% filter(listingDate>dates[i],listingDate<dates[i+1]) %>% mutate(temp_full = str_c(listingTitle,listingText))
  list <- frame$temp_full
  write_tokenized_sentences(list, file_names[i])
}
