...
## Social Forces R and R work

```{r}
load('cl_data_1_30_20.Rdata')
```


Reviewer 1 questioned about the possibility of overfitting. To address this, I'm re-fitting a model with no assumptions about prevalence, and then seeing how similar that model is to the model used for analysis.

First we re-fit a new, empty model with the same data and 40 topics, using the same seed. We can compare the cosine similarity for the topic vectors and show that the topics from each model are exceedingly similar: this means that in both models, the same words load highly onto each topic. So if 'rooftop' was important for topic 4 in the full model, it's not only important in the empty model, but it also is important to almost the same degree.
...
model_betas <- model$beta$logbeta
empty_betas <- empty_40$beta$logbeta
...
# function to compute cosine similarity of topics from various models, taken from ldaRobust package
sim_mat <- cos_sim(model_betas[[1]], empty_betas[[1]])

mean(diag(sim_mat))
```

Then we can compare the associations between topics and neighborhood covariates in the test set:

```{r}
n_topics = 40
effects_empty <- estimateEffect(formula = 1:n_topics ~ fct_relevel(fct_drop(neighborhood_type), c('predominantly white')), stmobj = empty_40, metadata = out$meta)
sum_empty <-  summary(effects_empty)
...
for(f_type in c('empty', 'full')){
  ...
  for(i in 1:n_topics){
    row <- tibble(topic = paste('Topic',i),
...
compare training associations in a plot: they look roughly similar 

```{r plot}
coef_sum <- bind_rows(test_coef_sum %>% mutate(type = 'old'), test_coef_sum_alt %>% mutate(type = 'alt'))
coef_sum %>% mutate(high_est = coefs + stderrs, low_est = coefs - stderrs, topic = factor(topic)) %>% 
  ...
  ggplot(aes(x = reorder(topic, -coefs), y = coefs))+
    ...
    ggtitle("Estimated Effects of neighborhood racial proportions on Topic Distributions - Comparing Full and Empty Models")+
    theme(text = element_text(size=10), #, # set values for text
...
Look at how often we match signs and signiifcance in the training set: there are a fair number of differences among coefficiencts that were insignificant in the full model, but relatively few differences for coefficients that were significant in the full model. There were 34 coefficients that were significant in one direction in the full model, but insignificant or with a significant sign change in the empty model. That was initially troubling to me, until I looked closer and saw that only 3 of those cases went from being significant in one direction to being significant in the other. That is, if we regard these two models as measurements of the same underlying process, then it's not especially surprising that in 10% of cases (25/240) the point estimate of a coefficient was different enough to change significance, or that in less than 4% of cases (9/240) a coefficient that was significant in teh full model changed sign in the empty model. Looking specifically at the 9 cases wehre a significant coefficient changed sign, only one of those was in a topic which I interpreted substantively, and in that case the sign and significance for the other 5 neighborhood types was unchanged. In other words, though not all of the 240 coefficients maintained their sings and significance under this test, the overall substantive interpretation of the two models is the same.

```{r}
#see how often things match up
coef_sum %>% mutate(sign = sign(coefs), sig = abs(coefs)>(2*stderrs)) %>% 
  #filter(sig)  %>% 
  pivot_wider(id_cols = c(topic, n_type), names_from = type, values_from = c(sign, sig)) %>%  
  mutate(sign_match = (sign_full == sign_empty), 
         sig_match = (sig_full ==sig_empty), 
         full_match = sign_match&sig_match) %>% 
  count(sig_full, sig_match, sign_match)
```


Here we can see the 9 rows that changed sign and significance
```{r compare rows}
#see how often things match up
coef_sum %>% mutate(sign = sign(coefs), sig = abs(coefs)>(2*stderrs)) %>% 
  #filter(sig)  %>% 
...
However, that was for the training set. Another way that I attempted to avoid overfitting was to use a test/validation set and reported the resutls from that set. So below, I use the empty model to estimate topic proportions for that set and compare the results, first by agian comparing cosine similarity of topic vectors (this time across documents instead of words). As before, results are very similar

```{r expand to test set}
empty_fit_average <- fitNewDocuments(model=empty_40, 
...
empty_fit_topics = make.dt(empty_fit_average) %>% inner_join(cl_test %>% mutate(docnum=as.numeric(rownames(cl_test))))

empty_topics <- empty_fit_topics %>% arrange(docnum) %>% select(starts_with('Topic')) %>% as.matrix() %>% t()
full_topics <- new_fit_topics %>% arrange(docnum) %>% select(starts_with('Topic')) %>% as.matrix() %>% t()


sim_mat <- cos_sim(empty_topics, full_topics)
mean(diag(sim_mat))
range(diag(sim_mat))
```

Now we'll compare the regression results across models as before, but on the test set:
...
empty_fit_topics['N_type_'] <- fct_relevel(fct_drop(new_fit_topics$neighborhood_type), c('predominantly white'))

rr_coef_sum <- tibble()
for(f_type in c('empty', 'full')){
  if(f_type == 'empty'){
    data <- empty_fit_topics
  } else {
    data <- new_fit_topics
  }
  for(i in paste0('Topic',1:40)){
    ...
    rr_coef_sum <- bind_rows(rr_coef_sum, row)
  }
}
...
Compared to the training set, the results for the test set show that the differences between the two approaches are even smaller when used out of sample: there are zero topic/neighborhood combinations which are significant in one direction in the full model and change to being sgnificant in the other direction in the empty model. 

```{r}
#see how often things match up
rr_coef_sum %>% mutate(sign = sign(coefs), sig = abs(coefs)>(2*stderrs)) %>% 
  #filter(sig)  %>% 
  pivot_wider(id_cols = c(topic, n_type), names_from = type, values_from = c(coefs, sign, sig)) %>%  
  mutate(sign_match = (sign_full == sign_empty), 
         sig_match = (sig_full ==sig_empty), 
         full_match = sign_match&sig_match,
         coef_diff = coefs_full - coefs_empty) %>% 
  count(sig_full, sign_match, sig_match)
  #filter(sig_full, !sign_match)
```

Reviewers also wanted some checks for spaical autocorrelation

```{r set up for spatial auto-correlation, results = "hide"}
library(INLA)
library(rgdal) # for readOGR
library(spdep) # for poly2nb and nb2inla
library(ape)

topic_sum <- new_fit_topics %>% 
  group_by(GEOID10) %>% 
  summarise_at(.vars = vars(contains('topic')), .funs = mean)

tract_map <- tracts_geo %>% st_as_sf() %>%
  select(GEOID) %>% 
  filter(!is.na(st_dimension(.$geometry))) %>% 
  inner_join(topic_sum, by = c('GEOID'='GEOID10'))

nb.map <- poly2nb(tract_map, row.names = tract_map$GEOID)

```

```{r morans i}
ww <-  nb2listw(nb.map, style='B', zero.policy = T)

moran_test <- tibble()
for(i in 1:40){
  test <- moran.test(tract_map[[paste0('Topic',as.character(i))]], ww, randomisation=FALSE, zero.policy = T)
  moran_test <- bind_rows(moran_test,
                           tibble(
                             topic = i,
                             stat = test$statistic,
                             p = test$p.value,
...
This was more of a pain to do than I though. INLA models take a long time to run, but with the Moran's I confriming that there is spatial autocorrelation, it was important to make sure that it's not ofsetting our results. I compared the coefficients for the INLA models the same way I compared the results from the check for data leakage. In this case, there were two coefficients that were negative and significant under the models I report and postiive and significant in the INLA model, as well as one that was positive and significan in my model and negative and significant in INLA. However, these coefficients were, not, I think, PRACTICALLY significant in either model.

More worrying is the fact that MANY coefficients that were significant in my model were insignificant under INLA, in other words, these coefficients seem to have only been significant because, possibly, of spatial confounding. HOwever, we know that the racial neighborhoods also have a spatial patterning, so it makes sense for these significances to shrink and it's not clear that soaking up that variation in a spatail error is the right choice.

```{r inla}

nb2INLA("watracts.graph",nb.map)

inla_data <- new_fit_topics %>% inner_join(tibble(tract_name = 1:length(tract_map$GEOID),
                                                  GEOID10 = tract_map$GEOID))

topic <- 'Topic1'

formula_bi <- log(get(topic)) ~ N_type_ + f(tract_name, model = 'bym2', graph = "watracts.graph")

formula <- log(get(topic)) ~ N_type_+
                     pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
                     ...
                     share_rental_over_20+
                     share_built_after_10+
                     share_built_before_40+
                     #median_gross_rent+
                     sf_detached+
                     log_price+
                     log_sqft+
                     log_dist_lr_m+
                     log_dist_sc_m+f(tract_name, model = 'bym2', graph = "watracts.graph")

inla_bi <- inla(formula_bi, data = inla_data, control.predictor = list(compute =TRUE))

inla_coef_sum <- tibble()
for(i in 1:n_topics){
  topic = paste0('Topic',i)
  inla_full <- inla(formula, data = inla_data, control.predictor = list(compute =TRUE))
  inla_sum <- summary(inla_full)
  
  inla_tibble <- as_tibble(inla_sum$fixed) %>% mutate(n_type = rownames(inla_sum$fixed))
  row <- inla_tibble %>% filter(str_detect(n_type, 'N_type')) %>% 
    transmute(n_type = str_remove(n_type, 'N_type_'),
              coef =`0.5quant`, 
              se = sd,
              sig = (sign(`0.025quant`)==sign(`0.975quant`)))
  inla_coef_sum <- bind_rows(inla_coef_sum, row)
  print(paste("completed", topic))
}

coef_sum_full <- bind_rows(test_coef_sum %>% mutate(type = 'full'), inla_coef_sum %>% mutate(type = 'inla'))

```

```{r}

inla_coef_sum %>% count(n_type)
coef_sum_full %>% count(n_type)

#see how often things match up
coef_sum_full %>% 
  filter(type == 'full') %>% 
  mutate(sign = sign(coefs), sig = abs(coefs)>(2*stderrs)) %>%  
  select(topic, n_type, sig, sign, type) %>% bind_rows(inla_coef_sum %>% 
                                                         transmute(topic = rep(paste0("Topic", 1:40), each = 6), 
                                                                   n_type = n_type, 
                                                                   sign = sign(coef), 
                                                                   sig = sig, 
                                                                   type = 'inla')) %>%
  pivot_wider(id_cols = c(topic, n_type), names_from = type, values_from = c(sign, sig)) %>%  
  mutate(sign_match = (sign_full == sign_inla), 
         sig_match = (sig_full ==sig_inla), 
         full_match = sign_match&sig_match,
         key_topic =as.numeric(str_extract(topic, '\\d+$')) %in% topics_to_examine) %>% 
  filter(key_topic) %>%
  #count(sig_full, sig_match, sign_match)
  #filter(sig_full, !sig_inla|!sign_match)
  group_by(topic) %>% count(!sig_full|(sig_inla&sign_match)) %>% arrange(`!sig_full | (sig_inla & sign_match)`, desc(n))

coef_sum_full %>% filter(type == 'full') %>% summarise(coef = mean(abs(coefs)))
```



## New Variables
Reviewers also wanted some more variables:

Share housing built before 1940
Median Housing Year

DONE - euclidean distance to Seattle city hall (proxy for proximity to downtown) 
- alternatively, manhattan distances to city hall by car and transit based on google maps
- proximity to k-12 schools (I want to leave this out, nobody asked for it)
DONE - ACS proportion of HU that are in structures that are single family detached
DONE Median rent
DONE distance to light rail
fixed intercpets?
exclude 'security depostis'
DONE population density

broken out vizes
viz with topic titles and top words
map included tracts vs Seattle CBSA


Repeat Analysis
DONE Check diff in results: NO significant changes 
DONE Check diff in model fit: BIC is LOWER In original models

```{r}
gmaps_api <- read_file('gmapskey.txt')
set.api.key(gmaps_api)
stations = read_lines('list_of_lr_stations.txt') %>% str_replace_all('\\s','+')

fake_distance <- function(origin, destination, mode){
  if(origin != destination){
    object = list('distance' = 100*str_length(destination))
    return(object)
  }
}

gmapsdistance(origin = '5410+Ivanhoe+PL+NE', destination = stations[1], mode = 'walking')$Distance
fake_distance(origin = '5410+Ivanhoe+PL+NE', destination = stations[1], mode = 'walking')$distance

address = '5410+Ivanhoe+PL+NE'

my_dist <- map_dbl(stations, .f = function(x) gmapsdistance(origin = address, destination = x, mode = 'walking')$Distance)

register_google(gmaps_api)
address_lat_lon <- geocode(address)

stations_lat_lon <- geocode(stations)

seattle_center <- geocode('seattle+WA')


stations_sf <- st_as_sf(stations_lat_lon %>% mutate(loc = read_lines('list_of_lr_stations.txt')), coords = c("lon", "lat"),
                        crs = "+proj=longlat +datum=WGS84")

seattle_center_sf <- st_as_sf(seattle_center %>% mutate(loc = 'seattle center'), coords = c("lon", "lat"),
                        crs = "+proj=longlat +datum=WGS84")

geo_df <- new_fit_topics %>% select(docnum, listingTitle, postID, lat, lng) %>% st_as_sf(coords = c("lng", "lat"),
                        crs = "+proj=longlat +datum=WGS84")

distances <- st_distance(stations_sf, geo_df, tolerance = 20)
min_dist <- map_dbl(1:ncol(distances), .f = function(x) min(distances[,x]))
new_fit_topics['dist_lr_m'] <- min_dist

distances <- st_distance(seattle_center_sf, geo_df, tolerance = 20)
min_dist <- map_dbl(1:ncol(distances), .f = function(x) min(distances[,x]))
new_fit_topics['dist_sc_m'] <- min_dist

mermerged_fit <- merged_fit %>% mutate(light_rail = str_detect(str_c(listingTitle,' ', listingText), regex('light\\s?rail', ignore_case = TRUE)),
                                            log_dist_sc_m = log(dist_sc_m),
                                            log_dist_lr_m = log(dist_lr_m))


# check if light rail findings hold up
ggplot(new_fit_topics, aes(dist_lr_m, as.numeric(light_rail), color = black_proportion>.1, group = black_proportion>.1))+
  geom_smooth()+
  xlim(0,6000)

ggplot(merged_fit %>% filter(dist_lr_m < 6000), aes(lng, lat, color = dist_sc_m))+
  geom_point(alpha = .1)+
  facet_wrap(~light_rail) 
```
...
map_sql <- "SELECT ST_Transform(f.geometry, 4326), seq_id, f.geoid, f.statefp, f.countyfp, listing_loc, listing_date, listing_title, listing_text, scraped_address, scraped_neighborhoods, clean_rent, clean_beds, clean_baths, clean_sqft, post_id, match_status, match_address, match_address2, geoparse_type,x,y
               FROM clean e
               RIGHT JOIN (
...
cl_full <- st_read(natrent, query = map_query)
cl_data <- cl_full
cl_data <- cl_data %>% #import cl data # maintain original indices
  mutate(py_index = 1:nrow(.),
         log_price = log(clean_rent),# make a logged price variable
         log_sqft = log(clean_sqft), 
         geoid = as.character(geoid)
         )# make a log square foot variable
 # make a copied df to use below (because we'll drop duplicates)
```  
...
acs_codes <- c('B03002_001E','B03002_003E','B03002_004E','B03002_005E','B03002_006E','B03002_007E','B03002_009E','B03002_012E','B17001_001E','B17001_002E','B19013_001E', 'B06009_005E','B25003_001E','B25003_002E','B25003_003E', 'B25032_001E','B25032_020E', 'B25032_021E', 'B25036_001E','B25036_015E','B25036_014E', 'B08012_002E', 'B08012_003E', 'B08012_004E', 'B08012_005E', 'B08012_006E', 'B08012_007E', 'B08012_008E', 'B08012_009E', 'B08012_010E', 'B08012_011E', 'B08012_012E', 'B08012_013E',	'B08006_002E','B25034_011E','B25035_001E', 'B25024_002E', 'B25031_001E')
acs_names <- c('total_RE','white', 'black', 'aindian', 'asian', 'pacisland', 'other', 'latinx', 'total_poverty', 'under_poverty', 'income', 'col_degree', 'total_tenure', 'owner_occupied', 'rental', 'total_tenure_size', 'rental_20_49','rental_over_50','total_tenure_year', 'built_10_13', 'built_after_14', 'commute_less_5_minutes', 'commute_5_to_9_minutes', 'commute_10_to_14_minutes', 'commute_15_to_19_minutes', 'commute_20_to_24_minutes', 'commute_25_to_29_minutes', 'commute_30_to_34_minutes', 'commute_35_to_39_minutes', 'commute_40_to_44_minutes', 'commute_45_to_59_minutes', 'commute_60_to_89_minutes', 'commute_90_or_more_minutes', 'commute_car','built_before_40', 'median_year_built','sf_detached', 'median_gross_rent')
...
tracts_17 <- get_acs(geography = 'tract', year = 2017, variables = acs_codes, output = 'wide', state = 'WA') %>% #grab census data
  select(-ends_with("M")) %>% mutate(all_other = aindian+pacisland+other) # drop moe columns

tracts <- bind_cols(tracts_17, 
                    setNames(tracts_17[re_cols]/tracts_17$total_RE,paste0(re_cols,"_proportion"))) %>% # make racial proportion columns
  mutate(pov_proportion = under_poverty/total_poverty, # poverty proportion variable
         ...
         share_built_before_40 = built_before_40/total_tenure, # share of units build in 1939 or earlier
         share_car_commuters = commute_car/total_RE,#share of car commuters
         share_college = col_degree/total_RE, #share of college education
         share_sf_detached = sf_detached/total_tenure,
         share_commute_over_20 = (commute_20_to_24_minutes + commute_25_to_29_minutes + commute_30_to_34_minutes + commute_35_to_39_minutes + commute_40_to_44_minutes + commute_45_to_59_minutes + commute_60_to_89_minutes + commute_90_or_more_minutes)/total_RE) #share of commuters with commutes over 20mins


tracts_geo <- tigris::tracts(state = 'WA')

tracts <- tracts %>% inner_join(tracts_geo@data %>% select(GEOID, land_area = ALAND), by = 'GEOID') %>% 
  mutate(land_area = as.numeric(land_area),
         pop_density = total_RE/land_area)

new_fit_topics <- inner_join(new_fit_topics[,!(names(new_fit_topics) %in% names(tracts))], tracts, by = c("GEOID10" = "GEOID"))  
``` 


```{r make neighborhood typology}
tract_type <- gather(tracts %>% 
...
cl_data <- inner_join(cl_data, tracts, by = c("geoid" = "GEOID"))
```
...
# drop urls
cl_data <- cl_data %>% mutate(dupeText = str_to_lower(listing_text), # convert to lower
                              ...
                              listing_text = str_replace_all(listing_text, 'QR Code Link to This Post', '') # remove this phrase from the listing text
                                                           ) %>% 
...
cl_data <- cl_data %>%
  mutate(textAndTitle = paste(listing_title, listing_text, sep='\n\n'),
...
interval = 50
n_topics = 40
cl_dropped <- cl_data %>% st_drop_geometry() %>% arrange(listing_date) %>% mutate(py_index = row_number())
...
drop_by_address <- function(top_100){
  # if top_100 is longer than 500, process in batches
  # take the first 500, 
...
addresses <- cl_dropped %>% drop_na(match_address) %>% count(match_address) %>% filter(n>10) %>% arrange(desc(n)) 
i = addresses$match_address[[1]]

address_total <- 0
for(i in addresses$match_address){
  top_100 <- cl_dropped %>% filter(match_address == i) %>% select(dupeText, py_index)
  while(nrow(top_100)>1000){
    print(paste("Too many listings for '",i,"'reducing and repeating"))
    top_100 <- head(top_100, 700)
    drop_list <- drop_by_address(top_100)
    cl_dropped <- cl_dropped %>% filter(!py_index %in% drop_list)
    address_total = address_total + length(drop_list)
    top_100 <- cl_dropped %>% filter(match_address == i) %>% select(dupeText, py_index)
  }
  # grab the top 100 documents matching each address
  drop_list <- drop_by_address(top_100)
    # drop them
  cl_dropped <- cl_dropped %>% filter(!py_index %in% drop_list)
...
While we were doing this work, ACS 2017 became available. THis next block makes a new version of the test data which has the new data attached. 

It seems like 15% of tracts actually changed neighborhood type

I also changed the commuting variable to use both the share of people who used cars and the share of commuters over 20 mins

```{r}
old_new_fit_topics <- new_fit_topics
mean((tracts %>% arrange(GEOID))$neighborhood_type == (tracts_16 %>% arrange(GEOID))$neighborhood_type)
key_cols <- names(tracts_16)[2:43]
new_fit_topics <- new_fit_topics %>% select(-key_cols, -neighborhood_type) %>% inner_join(tracts, by = c('GEOID10' = 'GEOID'))
...
if(file.exists(table_file)) file.remove(table_file)
model_list <- list()
new_fit_topics['N_type_'] <- fct_relevel(fct_drop(new_fit_topics$neighborhood_type), c('predominantly white'))
for(i in paste0('Topic',1:40)){
  test_model <- glm(log(get(i)) ~ N_type_+
                     ...
                     data = new_fit_topics)
  test_sum <- summary(test_model)
  write_lines((i), 'table.html', append = TRUE)
  write_lines(texreg::htmlreg(test_model), 'table.html', append = TRUE)
...
```{r summarize test results multivariable}
test_coef_sum_other <- data.frame()  
model_list <- list()
new_fit_topics['N_type_'] <- fct_relevel(fct_drop(new_fit_topics$neighborhood_type), c('predominantly white'))
for(i in paste0('Topic',1:40)){
  test_model <- glm(log(get(i)) ~ N_type_+
  ...
  test_coef_sum_other <- bind_rows(test_coef_sum_other, row)
}
``` 

```{r topic 19 interaction} 
topic_19_interaction <- glm(log(Topic19) ~ N_type_+
                     pov_proportion+ 
                     #N_type_*pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
                     share_car_commuters+
                     share_commute_over_20+ 
                     share_oo+
                     share_rental_over_20+
                     share_built_after_10+
                     log_price+
...
test_coef_sum_other %>%  
  ...
  ggplot(aes(x = reorder(topic, -coefs), y = coefs))+
    ...
    #geom_text(aes(y = -(sign(coefs))*.3 -(sign(coefs)*str_length(description)*.015), label = description), check_overlap = TRUE,  size = 4) +
    geom_text(aes(y = -(sign(coefs))*.45, label = description, hjust = 'outward'), size = 3.5, check_overlap = TRUE) +
    ylab("Log-Level Coefficient")+
    ...
    theme(
      #text = element_text(size=10), #, # set values for text
          axis.text.y = element_blank(),
          #axis.title.x = element_blank(),
          axis.title.y = element_blank()) +
    scale_color_discrete(name="Neighborhood Type", labels = c("Majority non-White", "Mixed", "White Asian", "White Black", "White Latinx", "White Mixed"))+
    scale_shape_discrete(name="Neighborhood Type", labels = c("Majority non-White", "Mixed", "White Asian", "White Black", "White Latinx", "White Mixed"))+
    ylim(min(test_coef_sum_bi$coefs)-.1,max(test_coef_sum_bi$coefs)+.1)+
    scale_x_discrete(expand = c(.05,.6))+
    #facet_wrap(~n_type)+
    coord_flip() 

#ggsave("output/plots/bivariable.png", height = 5, width = 9)
``` 
...
```{r calculate expected proportional increases}
test_topic = 'Topic26'
test_coef_sum_bi %>% mutate(percent = coefs*exp(coefs)*100) %>% filter(topic == test_topic) %>% arrange(percent)
test_coef_sum %>% mutate(percent = coefs*exp(coefs)*100) %>% filter(topic == test_topic) %>% arrange(percent)
```

# New Viz Method

fig 2: topic titles and words
1, 4, 7, 8, 16, 17, 19, 20, 23, 26, 28, 31, 32, 33, 39, 40

Fig 3: 33 and 40 regressions

Fig 4: 1 and 32 regressions

Fig  5: 28 and 33 regressions

Fig 6: 28 and 33 maps

Fig  7: 30 and 39 regressions

Fig 8: 30 and 39 maps

Fig 9: 8 and 17 regressions

Fig 10: 31, 4, 26 regressions



```{r}
topics_for_plot <- list('unit' = c(16,17,19,23), 'marketing' = c(1,4,31,32), 'nh' = c(8,20,26,28), 'access' = c(7,30,39,40))


for(plot in names(topics_for_plot)){
  print(plot)
  print(topics_for_plot[[plot]])
  save_plot(topics_for_plot[[plot]], paste0(c("plots/",plot,".png"), collapse = ''))
}
save_plot(c(33,40, 1,32,4))
...
topics_to_examine <- c(4,33)
save_plot <- function(topics_to_examine, filename = NULL){
  plot_df <-  test_coef_sum %>%  
    left_join(read_csv('resources/topic_descriptions.txt'), by =  c('topic', 'n_type')) %>%
    mutate(high_est = coefs + 2*stderrs, low_est = coefs - 2*stderrs, topic = factor(str_extract(topic, '\\d+'))) %>%
    filter(topic %in% topics_to_examine) %>%
    group_by(topic) %>% mutate(adjust = row_number(desc(coefs)), y = -mean(coefs)) %>% ungroup() %>% mutate(y = rank(y, ties.method = 'min')*1.4)
    

  p <- plot_df %>%  ggplot(aes(x = y+adjust, y = coefs))+
      geom_hline(yintercept = 0, color = "red", alpha=.5) + # plot a line at 0
      geom_pointrange(aes(ymax = high_est, ymin = low_est, color=n_type, shape = n_type), size=.5, alpha = .8)+
      geom_text(aes(x = y+3.5, y = -(sign(coefs))*.35, label = description, hjust = 'outward'), size = 3.5, check_overlap = TRUE) +
      ylab("Log-Level Coefficient")+
      theme_minimal() + # auto exlude backgound shading
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
      theme(
            axis.text.y = element_blank(),
            axis.title.y = element_blank()) +
      scale_color_manual(name="Neighborhood Type", labels = c("Majority non-White", "Mixed", "White Asian", "White Black", "White Latinx", "White Mixed"), values = cbbPalette)+
      scale_shape_discrete(name="Neighborhood Type", labels = c("Majority non-White", "Mixed", "White Asian", "White Black", "White Latinx", "White Mixed"))+
      ylim(min(test_coef_sum_bi$coefs)-.1,max(test_coef_sum_bi$coefs)+.1)+
      scale_x_discrete(expand = c(.05,.6))+
      coord_flip() 
  if(is.null(filename)){
     filename <- paste0("output/plots/plot",paste(topics_to_examine, collapse = '_'), ".png")
  }
  ggsave(filename = filename, plot = p, height = 5, width = 9)
  p
}
...
Appendix table
```{r}
test_coef_sum %>% left_join(read_csv('resources/topic_descriptions.txt') %>% select(-n_type), by =  c('topic')) %>%
  mutate(coef_se = str_c(round(coefs, 3),' (', round(stderrs,3), ')')) %>% 
  select(-coefs, -stderrs, - topic) %>%
  pivot_wider(names_from = n_type, values_from = coef_se) %>% kable()
...
```{r} 
# for those in the paper
topics_to_label <- c(1, 4, 7, 8, 16, 17, 19, 20, 23, 26, 28, 31, 32, 33, 39, 40)

# for all topics
topics_to_label <- 1:40
labels <- labelTopics(model, topics_to_label, n= 6)
joined_sum <- test_coef_sum %>%  
    left_join(read_csv('resources/topic_descriptions.txt'), by =  c('topic', 'n_type')) %>%
  mutate(topic_num = str_extract(topic, '\\d+')) %>% filter(!is.na(description))

word_tibble <- as_tibble(labels$frex) %>% transmute(topic_num = as.character(row_number()), frex_words = str_c(V1, V2, V3, V4, V5, V6, sep = ', '))

joined_sum %>% inner_join(word_tibble) %>% filter(topic_num %in% topics_to_label) %>% select(description, frex_words) %>% kable()
```
...
merged_fit <- merged_fit %>% mutate(
  safety_terms = str_detect(str_remove_all(rawcleanText, regex('security deposit', ignore_case = TRUE)), regex("secured|security|patrol|gated", ignore_caes = TRUE)),
...
terms_model_kc_bi <- glm(safety_terms ~ `Neighborhood Type:`, data = merged_fit %>% filter(str_detect(GEOID10,'^53033')), family = binomial())

terms_model_kc <- glm(safety_terms ~ `Neighborhood Type:`+
                     pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
                     ...
                     share_built_before_40+
                     median_gross_rent+
                     sf_detached, data = merged_fit %>% filter(str_detect(GEOID10,'^53033')), family = binomial())

terms_model_kc_old <- glm(safety_terms ~ `Neighborhood Type:`+
                     pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
                     ...
                     share_oo+
                     share_rental_over_20+
                     share_built_after_10, data = merged_fit %>% filter(str_detect(GEOID10,'^53033')), family = binomial())

safety_terms <- c("secure", "secured", "security", "control", "controlled", "patrol", "gate", "gated", "protect", "protection", "protected", "intercom", "alarm")

keyword <- safety_terms[1]
find_keyword <- function(keyword){
  merged_fit['keyword_term'] <- str_detect(merged_fit$rawcleanText, keyword)
  merged_fit  %>% filter(str_detect(GEOID10,'^53033')) %>% count(neighborhood_type, keyword_term) %>% 
  mutate(term = keyword, full_prop = sum(if_else(keyword_term, n, 0L)/sum(n))) %>%  group_by(neighborhood_type) %>% mutate(prop = n/sum(n))
}
keyword_test <- map_dfr(safety_terms, function(x) find_keyword(x))

keyword_test %>% mutate(more = prop-full_prop) %>% filter(keyword_term==TRUE) %>% arrange(neighborhood_type != 'white black', desc(more))

summary(terms_model_kc)
stargazer(test_model, type = 'text')

merged_fit  %>% filter(str_detect(GEOID10,'^53033')) %>% count(neighborhood_type, safety_terms) %>% 
  mutate(full_prop = sum(if_else(safety_terms, n, 0L)/sum(n))) %>%  group_by(neighborhood_type) %>% mutate(prop = n/sum(n))
```
...
#library(zipcode)
library(censusr)
load("resources/zipcode.rda")
run_fields <- c("street",'city','state')
address_match <- cl_data

kc_AptComplex <- read_csv("cd/EXTR_AptComplex.csv") %>% 
  mutate(zip = str_extract(Address, '\\d+$'), 
         street = str_remove(Address,'\\s\\d+$')) %>% 
  drop_na(zip) %>% merge(zipcode)
kc_CondoComplex<- read_csv('cd/EXTR_CondoComplex.csv')%>% mutate(zip = str_extract(Address, '\\d+$'), street = str_remove(Address,'\\s\\d+$')) %>% drop_na(zip) %>% merge(zipcode)
 

kc_data <- bind_rows(kc_AptComplex %>% select(SectySystem, run_fields), kc_CondoComplex %>% select(SectySystem, run_fields))

source('geocoding.R')
outcome <- read_sf('resources/kctax_geocoded_2.shp')
kc_merged <- arcpyGeocode(kc_data, run_fields, run_no = 1)
tbl <- kc_data
st_crs(outcome)
st_transform(wa_shape, crs =st_crs(outcome))
x <- st_join(outcome, st_transform(wa_shape, crs =st_crs(outcome)))
kc_data <- inner_join(kc_data, st_drop_geometry(x))
```


```{r prep security regress}
kc_tracts <- kc_data %>% inner_join(tracts, by = c('GEOID10'='GEOID')) 


kc_tracts <- kc_tracts %>% mutate(
  SectySystem = ifelse(SectySystem=='Y',1,SectySystem),
  SectySystem = ifelse(SectySystem=='N',0,SectySystem),
  SectySystem = as.numeric(SectySystem)) %>% drop_na(SectySystem)
...
tax_model <- glm(SectySystem ~ `Neighborhood Type:`+
                     ...
                     share_built_before_40+
                     median_gross_rent+
                     sf_detached, data = kc_tracts, family = 'binomial')

tax_model_old <- glm(SectySystem ~ `Neighborhood Type:`+
                     pov_proportion+
                     log_income+
                     pop_thousands+
                     share_college+
...
texreg::screenreg(c(terms_model_kc, tax_model))
```

```{r security plot}
tax_sum <- summary(tax_model)$coefficients
terms_sum <- summary(terms_model_kc)$coefficients
...
#ggsave('output/plots/securityplot.png')
```
...
cbbPalette_kc <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
kc_cl_26 <- merged_fit %>% filter(GEOID10 %in% kc_nt$Tract_FIPS, Topic26>.1)
...
ggplot() + 
  ...
  #            color = 'red',
  #            alpha = 1)+
  coord_sf(xlim=c(-122.5,-122),
           ylim=c(47.77,47.15),
           datum = NA) +
  theme(legend.position = 'right', rect = element_blank()) +
  scale_fill_manual(name="Neighborhood Type", labels = c("Majority non-White", "Mixed", "White Asian", "White Black", "White Latinx", "White Mixed"), values = cbbPalette_kc)+
  theme(text = element_text(size=10), #, # set values for text
        axis.text.y = element_blank(),
        axis.title.x = element_blank(),
        axis.title.y = element_blank())+
  ggtitle('"Safe and Friendly" (26)')


ggsave('plots/cl_26_legend.png', device = png())
```
...
```{r}
savannah_texts <- NULL
savannah_texts <- bind_rows(merged_fit %>% filter(Topic4>.3) %>% arrange(aindian_proportion) %>% tail(10) , savannah_texts)

# function for simply exporting texts
file <-  'output/savanna_texts_2.txt'
#texts <- cl_data %>% filter(aindian_proportion>.1)
texts <- savannah_texts
for(i in 1:nrow(texts)){
  row = texts[i,]
  meta = paste("This text was from a listing at", row$matchAddress, row$matchAddress2, "with", round(row$aindian_proportion, 4), "proportion of Native folks.\nTopic 4: ", round(row$Topic4,4),"\nTopic 2: ", round(row$Topic2, 4))
