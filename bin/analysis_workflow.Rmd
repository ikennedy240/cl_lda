...
tracts_no_geo <- sf::st_drop_geometry(tracts_17)

tracts <- bind_cols(tracts_17, setNames(tracts_no_geo[re_cols]/tracts_no_geo$total_RE,paste0(re_cols,"_proportion"))) %>% # make racial proportion columns
...
terms_model_kc <- glm(safety_terms ~ `Neighborhood Type:`+
                     ...
                     share_oo+
                     share_rental_over_20+
                     share_built_after_10, data = merged_fit %>% select(-key_cols, -neighborhood_type) %>% inner_join(tracts, by = c('GEOID10' = 'GEOID')) %>% filter(str_detect(GEOID10,'^53033')), family = binomial())
...
kc_files <- c('EXTR_AptComplex.csv', 'EXTR_CondoComplex.csv',)
...
kc_CondoComplex<- read_csv('cd/EXTR_ResBldg.csv')%>% mutate(zip = str_extract(Address, '\\d+$'), street = str_remove(Address,'\\s\\d+$')) %>% drop_na(zip) %>% merge(zipcode)
...
ggplot() + 
  ...
  scale_fill_brewer("Neighborhood\nTypes",
                    type= 'qual',
                    palette = 'Set1',
                    direction =1, labels = c("Mixed", "Predominantly White", "White Asian", "White Black", "White Latinx", "White Mixed", "Majority non-White"))+
  theme(text = element_text(size=10), #, # set values for text
        axis.text.y = element_blank(),
        axis.title.x = element_blank(),
        axis.title.y = element_blank())+
  ggtitle('“Commuting Distance” (39)')


ggsave('output/plots/cl_39_legend.png', device = png())
```
