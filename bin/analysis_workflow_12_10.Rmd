...
tract_map <- tracts_17 %>% 
  select(GEOID) %>% 
  filter(!is.na(st_dimension(tracts_17$geometry))) %>% 
...
formula <- log(get(topic)) ~ N_type_+
                       ...
                       share_built_after_10+
                       log_price+
                       log_sqft+f(tract_name, model = 'bym2', graph = "watracts.graph")
...
#see how often things match up
coef_sum_full %>% filter(type == 'full') %>% mutate(sign = sign(coefs), sig = abs(coefs)>(2*stderrs)) %>% 
  select(topic, n_type, sig, sign, type) %>% bind_rows(inla_coef_sum %>% 
                                                         transmute(topic = rep(paste("Topic", 1:40), each = 6), 
                                                                   n_type = n_type, 
                                                                   sign = sign(coef), 
                                                                   sig = sig, 
                                                                   type = 'inla')) %>%
...
new_fit_topics <- new_fit_topics %>% mutate(light_rail = str_detect(str_c(listingTitle,' ', listingText), regex('light\\s?rail', ignore_case = TRUE)))


# check if light rail findings hold up
ggplot(merged_fit, aes(dist_lr_m, as.numeric(light_rail), color = black_proportion>.1, group = black_proportion>.1))+
  geom_smooth()+
  xlim(0,6000)
...
acs_codes <- c('B03002_001E','B03002_003E','B03002_004E','B03002_005E','B03002_006E','B03002_007E','B03002_009E','B03002_012E','B17001_001E','B17001_002E','B19013_001E', 'B06009_005E','B08015_001E','B25003_001E','B25003_002E','B25003_003E', 'B25032_001E','B25032_020E', 'B25032_021E', 'B25036_001E','B25036_015E','B25036_014E', 'B25034_011E','B25035_001E', 'B25024_002E', 'B25031_001E')
acs_names <- c('total_RE','white', 'black', 'aindian', 'asian', 'pacisland', 'other', 'latinx', 'total_poverty', 'under_poverty', 'income', 'col_degree','commute', 'total_tenure', 'owner_occupied', 'rental', 'total_tenure_size', 'rental_20_49','rental_over_50','total_tenure_year', 'built_10_13', 'built_after_14', 'built_before_40', 'median_year_built','sf_detached', 'median_gross_rent')
...
tracts_17 <- get_acs(geography = 'tract', year = 2017, output = 'wide', variables = acs_codes, state = 'WA') #grab census data

tracts_17 <- tracts_17 %>% select(-ends_with("M")) %>% mutate(all_other = aindian+pacisland+other) # drop moe columns



tracts <- bind_cols(tracts_17, setNames(tracts_17[re_cols]/tracts_17$total_RE,paste0(re_cols,"_proportion"))) %>% # make racial proportion columns
...
merged_fit <- inner_join(merged_fit[,!(names(merged_fit) %in% names(tracts))], tracts, by = c("GEOID10" = "GEOID")) 
```
...
## Reviewers asked us to respecificy the model with some new variables
DONE - euclidean distance to Seattle city hall (proxy for proximity to downtown) 
- alternatively, manhattan distances to city hall by car and transit based on google maps
- proximity to k-12 schools (I want to leave this out, nobody asked for it)
DONE - ACS proportion of HU that are in structures that are single family detached
...
```{r summarize test results New Variables}
test_coef_sum_alt <- data.frame()  
model_list <- list()
new_fit_topics['N_type_'] <- fct_relevel(fct_drop(new_fit_topics$neighborhood_type), c('predominantly white'))
for(i in paste0('Topic',1:40)){
  test_model <- glm(log(get(i)) ~ N_type_+
  ...
  test_coef_sum_alt <- bind_rows(test_coef_sum, row)
}
```

```{r compare bivarate and multi-variable analysis}
model_compare <- test_coef_sum %>% inner_join(test_coef_sum_alt, by = c('topic', 'n_type')) %>% mutate(same_sign = sign(coefs.x) == sign(coefs.y), diff = coefs.x - coefs.y, absdiff = abs(diff), diffsign = absdiff*(same_sign==FALSE))
model_compare %>% filter(absdiff > 0)
model_compare %>% group_by(topic) %>% summarize_at(c('bic.x', 'bic.y'), first) %>% count(bic.x>bic.y)
...
merged_fit <- merged_fit %>% mutate(
  safety_terms = str_detect(str_remove_all(rawcleanText, regex('security deposit', ignore_case = TRUE)), regex("secure|secured|security|control|controlled|patrol|gate|gated|protect|protection|protected|intercom|alarm", ignore_caes = TRUE)),
...
terms_model <- glm(safety_terms ~ `Neighborhood Type:`+
                     ...
                     share_built_before_40+
                     median_gross_rent+
                     sf_detached, data = merged_fit, family = binomial())
summary(terms_model)
...
kc_tracts <- inner_join(kc_tracts[,!(names(kc_tracts) %in% names(tracts))] %>% mutate(geoid = as.character(geoid)), tracts, by = c("geoid" = "GEOID")) 
